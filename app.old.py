...
def step_3(model, detection_threshold, verification_threshold):
    ...
    if verify_btn:
        with st.spinner('Verifying...'):
            ...
            if verified == True:
                st.success("Successfully Verified! We detected you as the : " + str(detection) +
                           "th person in the verification images directory")
                clockin(username)
            else:
                st.error('User is not defined. Please register as a new user')
